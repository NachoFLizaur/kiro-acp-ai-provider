...
var IPCServerImpl = class {
  ...
  constructor(options = {}) {
    ...
    this.secret = randomBytes2(32).toString("hex");
  }
  async start() {
    if (this.server) {
...
var ACPClient = class _ACPClient {
   ...
   */
  instanceId = randomBytes3(4).toString("hex");
  ...
  setupAgentConfig(populatedToolsFilePath) {
    const bridgePath = this.resolveBridgePath();
    const makeTmpPath = (path) => `${path}.${process.pid}.${randomBytes3(4).toString("hex")}.tmp`;
    let toolsFile;
    if (populatedToolsFilePath) {
      ...
      const tmpPath = makeTmpPath(toolsFile);
      writeFileSync2(tmpPath, JSON.stringify(toolsData, null, 2), { mode: 384 });
      renameSync2(tmpPath, toolsFile);
  ...
  getAgentConfigLockPath() {
    ...
    const sanitizedAgent = this.options.agent.replace(/[^a-zA-Z0-9_-]/g, "_");
    return join3(this.options.cwd, ".kiro", "agents", `${sanitizedAgent}.lock`);
  }
  ...
  resolveBridgePath() {
    ...
    const binDir = dirname2(process.argv[0] || "");
    if (binDir) {
      let dir = binDir;
      for (let i = 0; i < 10; i++) {
...
function persistSession(cwd, sessionId, affinityId) {
  try {
    ...
    writeFileSync3(tmpPath, JSON.stringify(data), { mode: 384 });
...
var KiroACPLanguageModel = class _KiroACPLanguageModel {
  ...
  async acquireSession(tools) {
    ...
    if (tools && tools.length > 0) {
      const streamId = randomBytes4(4).toString("hex");
...
Please acknowledge this context and continue from where we left off.
  ...
  writeToolsToFile(toolsFilePath, tools) {
    ...
    writeFileSync4(tmpPath, JSON.stringify(toolsData, null, 2), { mode: 384 });
    renameSync4(tmpPath, toolsFilePath);
    return toolNames;
  }
  syncToolsToBridgePath(sessionToolsFilePath) {
    ...
    try {
      const content = readFileSync4(sessionToolsFilePath, "utf-8");
      const tmpPath = bridgePath + ".tmp";
      writeFileSync4(tmpPath, content, { mode: 384 });
      renameSync4(tmpPath, bridgePath);
    } catch {
    }
  }
  ensureIpcPortInToolsFile(toolsFilePath) {
    ...
    try {
      ...
      const tmpPath = toolsFilePath + ".tmp";
      writeFileSync4(tmpPath, JSON.stringify(parsed, null, 2), { mode: 384 });
      renameSync4(tmpPath, toolsFilePath);
    } catch {
    }
  }
